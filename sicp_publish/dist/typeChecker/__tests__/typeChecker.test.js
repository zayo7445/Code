...
function parseAndTypeCheck(code, chapterOrContext = types_1.Chapter.SOURCE_1) {
    const context = typeof chapterOrContext === 'number' ? (0, context_1.mockContext)(chapterOrContext) : chapterOrContext;
    const program = (0, parser_1.parse)(code, context);
    expect(program).not.toBeUndefined();
    const validatedProgram = (0, validator_1.validateAndAnnotate)(program, context);
    return (0, typeChecker_1.typeCheck)(validatedProgram, context);
}
function topLevelTypesToString(program) {
    return program.body
        .filter(node => ['VariableDeclaration', 'FunctionDeclaration'].includes(node.type))
        .map((node) => {
        var _a;
        const id = node.type === 'VariableDeclaration'
            ? node.declarations[0].id.name
            : (_a = node.id) === null || _a === void 0 ? void 0 : _a.name;
        const actualNode = node.type === 'VariableDeclaration'
            ? node.declarations[0].init
            : node;
        const type = actualNode.typability === 'Untypable'
            ? "Couldn't infer type"
            : (0, stringify_1.typeToString)(actualNode.type === 'FunctionDeclaration'
...
describe('type checking pairs and lists', () => {
    ...
    it('happy paths for list functions', () => {
        const context = (0, context_1.mockContext)(types_1.Chapter.SOURCE_2);
    ...
    `);
        expect((0, index_1.parseError)(errors1)).toMatchInlineSnapshot(`
    ...
    `;
      ...
      xs3: List<boolean>"
    `);
        expect((0, index_1.parseError)(errors2)).toMatchInlineSnapshot(`""`);
    });
    ...
    `;
        const [program, errors] = parseAndTypeCheck(code, types_1.Chapter.SOURCE_2);
        expect(topLevelTypesToString(program)).toMatchInlineSnapshot(`
      "accumulate: ((T0, T1) -> T1, T1, List<T0>) -> T1
      xs: List<number>
      ys: List<boolean>"
    `);
        expect((0, index_1.parseError)(errors)).toMatchInlineSnapshot(`""`);
    });
    ...
    `);
        expect((0, index_1.parseError)(errors, true)).toMatchInlineSnapshot(`
...
describe('type checking functions', () => {
    ...
    `;
        const [program, errors] = parseAndTypeCheck(code1, types_1.Chapter.SOURCE_2);
        expect(topLevelTypesToString(program)).toMatchInlineSnapshot(`"append: (List<T0>, List<T0>) -> List<T0>"`);
        expect((0, index_1.parseError)(errors)).toMatchInlineSnapshot(`""`);
    });
    ...
    `;
        ...
        expect((0, index_1.parseError)(errors)).toMatchInlineSnapshot(`"Line 3: foo contains cyclic reference to itself"`);
        expect((0, index_1.parseError)(errors, true)).toMatchInlineSnapshot(`
...
describe('type checking for polymorphic builtin functions', () => {
    ...
    it('errors in unhappy path', () => {
      ...
      x + 5;
    `;
        const [, errors] = parseAndTypeCheck(code, types_1.Chapter.SOURCE_1);
        expect((0, index_1.parseError)(errors)).toMatchInlineSnapshot(`
      "Line 3: A type mismatch was detected in the binary expression:
        x + 5
      The binary operator (+) expected two operands with types:
...
describe('type checking overloaded unary/binary primitives', () => {
    it('works for the happy path', () => {
      ...
      const y = x || true;
    `;
        const [program, errors] = parseAndTypeCheck(code, types_1.Chapter.SOURCE_1);
        expect((0, index_1.parseError)(errors)).toMatchInlineSnapshot(`""`);
        expect(topLevelTypesToString(program)).toMatchInlineSnapshot(`
      "foo: number -> number
      bar: (number, number) -> number
...
describe('Type checking reassignment for Source 3', () => {
    ...
    `;
        const [program, errors] = parseAndTypeCheck(code1, types_1.Chapter.SOURCE_3);
...
describe('checking while loops in source 3', () => {
    ...
    `;
        const [program, errors] = parseAndTypeCheck(code, types_1.Chapter.SOURCE_3);
...
describe('Type context from previous executions get saved', () => {
    ...
    it('source 2 list functions', () => __awaiter(void 0, void 0, void 0, function* () {
        const code1 = `
    ...
    `;
        const context = (0, context_1.mockContext)(types_1.Chapter.SOURCE_2);
        yield (0, index_1.runInContext)('', context); // we run an empty program to simulate execution of prelude
    ...
    it('source 3 stream functions', () => __awaiter(void 0, void 0, void 0, function* () {
        const code1 = `
...
describe('imported vars have any type', () => {
    ...
    `;
        const [program, errors] = parseAndTypeCheck(code1, types_1.Chapter.SOURCE_1);
        expect(topLevelTypesToString(program)).toMatchInlineSnapshot(`
      "a: T0
      b: T0
      c: T0
