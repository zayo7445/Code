...
test('display_list is linear runtime', () => {
    ...
    const noDisplayList = (v, s = placeholder) => {
        if (s !== placeholder && typeof s !== 'string') {
            throw new TypeError('display_list expects the second argument to be a string');
        }
        return list.rawDisplayList((x) => x, v, s === placeholder ? undefined : s);
    };
    return expect((0, testing_1.testSuccess)((0, formatters_1.stripIndent) `
      const build_inf = (i, f) => {
        const t = list(f(i));
        let p = t;
    ...
    }).then(testResult => testResult.result)).resolves.toBeLessThan(1.2);
    // estimated power is less than 1.2
    // means it's probably near 1
    // => probably linear?
}, 1000000);
//# sourceMappingURL=list-benchmark.js.map